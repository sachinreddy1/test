    ...
    override var itemList: List<C> = items ?: listOf()
        set(value) {
            if (!itemList.isNullOrEmpty()) {
                println("-------")
                val testA = (itemList.first() as MutableList<Cell>).first().data
                println("A: $testA")

                val testB = (value.first() as MutableList<Cell>).first().data
                println("B: $testB")
            }
...
//            println(newList.map { it.data.size })
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AbstractViewHolder {
        ...
        val recyclerView: CellRecyclerView = CellRecyclerView(context)
